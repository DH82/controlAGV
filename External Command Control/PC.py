...
# 전역 변수
stream_buffer = None
current_command = None  # 현재 QR 코드를 처리할 때 사용할 명령을 저장하는 전역 변수
...
def process_image():
    global current_command, stream_buffer
    ...
    if current_command:
        response["command"] = current_command
        current_command = None  # 명령을 전송한 후 전역 변수를 초기화
...
def command():
    ...
    if ',' in data:
        ...
        try:
            ...
            if (x, y) == (1, 1):
                current_command = "(1,1)"  # 좌표가 (1,1)인 경우 특정 명령으로 설정
        except ValueError:
        ...
        if data in ['1', '2', '3']:
            current_command = data  # '1', '2', '3' 중 하나의 신호를 받은 경우 해당 값을 명령으로 설정
